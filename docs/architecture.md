...
Markdown/HTML snapshots are generated from a consistent read. Content escapes HTML, note paths use stable UUIDs, and original source files are never rewritten. The snapshot identity includes the record heads and export-renderer version, so renderer fixes create a new immutable directory. Different existing export contents cause a conflict whose diagnostics name files but do not print private contents. Unix staging uses restrictive modes; Windows staging and nested directories inherit the reviewed data-home ACL so explicitly authorized owner and sandbox identities can reopen the same export. Snapshots retain history on disk and need owner-directed retention management for a large corpus.
